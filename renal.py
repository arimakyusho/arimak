...
def generate_patient_report(patient_id):
    try:
        ...
        pdf.set_font("Arial", 'B', 14)
        pdf.cell(0, 10, "Diagnostic Tests", 0, 1, 'L')
        pdf.ln(5)
        ...
        pdf.set_y(-15)
        pdf.set_font("Arial", 'I', 8)
        pdf.cell(0, 5, f"Generated by: {st.session_state.full_name} ({st.session_state.username}) | Renal Tracker Pro", 0, 0, 'C')
...
def show_patient_details(patient_id):
    ...
    tab1, tab2, tab3 = st.tabs(["Overview", "Medications", "Diagnostics"])
